			...
			handler: async (args) => {
				...
				try {
					if (ci) {
						...
						await processTask({ taskId, jobToken: process.env.NJUST_AI_CJ_CLOUD_TOKEN || null })
					} else {
...
if (!fs.existsSync(EVALS_REPO_PATH)) {
	console.error(
		`Exercises do not exist at ${EVALS_REPO_PATH}. Please run "git clone https://github.com/NJUST-AI/NJUST_AI_CJ-Evals.git evals".`,
	)
