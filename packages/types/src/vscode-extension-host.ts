...
/**
 * All known webview message types.
 *
 * MUST be kept in sync with `WebviewMessage.type`.  When you add a new message
 * type to the interface above, add its string literal here as well — otherwise
 * the Zod runtime validator will reject it at the webview boundary.
 */
...
/**
 * Zod schema that validates a known webview message type at runtime.
 */
export const webviewMessageTypeSchema = z.enum(WEBVIEW_MESSAGE_TYPES)
 ...
 * success or `null` when the type is unknown / missing.
 */
export function safeParseWebviewMessage(raw: unknown): WebviewMessage | null {
	const result = webviewMessageSchema.safeParse(raw)
	if (result.success) {
		return result.data as WebviewMessage
	}
	return null
}
