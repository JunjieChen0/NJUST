...
| 功能 | 说明 |
|------|------|
| **`.njust_ai/mcp.json` / `mcp_settings.json`** | 扩展作为 **MCP 客户端**，连接**外部**仓颉 MCP 服务，工具出现在对话里（`mcp_*`）。 |
...
复制内容到 `.njust_ai/mcp.json` 后，按你的实际路径与启动命令修改即可。

## 7. 仍觉得能力不够时

- 在 **仓颉模式**下使用项目内 `.njust_ai/skills/` 与系统提示中的工程上下文（`cjpm.toml`、包结构等）。  
