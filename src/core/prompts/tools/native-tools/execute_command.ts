...
Before proposing compile/build/run commands (e.g. gcc/clang, running a freshly built binary), read the **# Host System** section inside the **Environment Details** block in the latest user message: it states the detected OS and platform-specific patterns (Windows vs macOS vs Linux) so you choose commands that actually work on this machine.
