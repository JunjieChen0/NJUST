...
任务事件系统采用 TypeScript 类型安全的设计，通过 `NJUST_AI_CJEventName` 枚举统一管理所有事件类型，并通过 `TaskEvents` 接口定义事件参数规范。系统支持异步事件处理、错误处理和事件传播机制，确保任务状态变化能够及时通知到各个监听器。
...
Events[事件定义<br/>NJUST_AI_CJEventName]
...
graph TB
subgraph "事件定义层"
EventEnum[NJUST_AI_CJEventName<br/>事件枚举]
EventSchema[rooCodeEventsSchema<br/>事件模式]
...
```typescript
// 正确的事件监听器注册方式
task.on(NJUST_AI_CJEventName.TaskStarted, (taskId) => {
...
task.on(NJUST_AI_CJEventName.TaskStarted, handleTaskStarted);
```
...
```typescript
// 使用类型断言确保参数类型正确
task.on(NJUST_AI_CJEventName.TaskCompleted, ([taskId, tokenUsage, toolUsage]) => {
...
task.on(NJUST_AI_CJEventName.TaskStarted, listener);

// 在适当的时候移除监听器
// task.off(NJUST_AI_CJEventName.TaskStarted, listener);

// 或者使用一次性监听器
task.once(NJUST_AI_CJEventName.TaskCompleted, () => {
    // 只执行一次的逻辑
});
```
...
通过持续的优化和完善，任务生命周期事件系统将为 Roo-Code 平台提供更加稳定可靠的任务管理能力。