...
## 简介
本文件系统性梳理 Roo-Code 项目的整体技术栈与依赖，涵盖 TypeScript/JavaScript、React/Next.js、VS Code 扩展开发、Monorepo 架构与 Turbo 构建系统等。文档同时说明关键技术选型原因、版本要求与配置要点，列举关键第三方依赖库及其作用，并提供学习路径与参考资料。

## 项目结构
Roo-Code 采用 pnpm workspaces + Turbo 的 Monorepo 架构，核心目录与职责如下：
- apps：多应用入口
  - web-roo-code：Next.js 官网首页与内容站点
...
subgraph "应用 apps/"
WEB_ROO["web-roo-code<br/>Next.js"]
...
ESL["@njust-ai-cj/config-eslint"]
TSCFG["@njust-ai-cj/config-typescript"]
end
PKG --> WS
PKG --> TURBO
PKG --> TSC
...
- React/Next.js 前端
  ...
  - web-roo-code：Next.js 16，TailwindCSS，Radix UI，用于官网与内容站点。
...
## 架构总览
Roo-Code 的运行时逻辑分层清晰：Webview 与扩展宿主分离，宿主内 ClineProvider/Task 串联 UI、模型与各类服务。AI 模型通过统一抽象接入多家提供商，服务层包含 MCP、代码索引、Cloud Agent、Skills、仓颉 LSP 等。
...
Plugins --> Resolve["别名解析<br/>@ -> src, @roo -> ../src/shared"]
...
### Next.js 应用（web-roo-code 与 web-evals）
- web-roo-code
...
- 配置包
  - @njust-ai-cj/config-typescript：统一 TS 配置。
  - @njust-ai-cj/config-eslint：统一 ESLint 配置（含 React Hooks、Turbo、Prettier 等插件）。
...
- 搜索服务
  - 自然语言查询命中向量库，供 Agent 工具 codebase_search 与 VS Code 注册的 roo_codebaseSearch 使用。
- 缓存
...
- 关键依赖分布
  ...
  - web-roo-code：Next.js、TailwindCSS、Radix UI、Framer Motion、Zod、PostHog、Next Themes 等。
...
WEB_ROO_PKG["web-roo-code/package.json"]
WEB_EVALS_PKG["web-evals/package.json"]
CLI_PKG["apps/cli/package.json"]
end
subgraph "共享包"
TYPES_PKG["@njust-ai-cj/types/package.json"]
ESL_PKG["@njust-ai-cj/config-eslint/package.json"]
TSCFG_PKG["@njust-ai-cj/config-typescript/base.json"]
end
PKG --> WS
PKG --> TURBO
WS --> WV_PKG
...
## 结论
Roo-Code 采用现代前端与 Node 生态，结合 VS Code 扩展与 Monorepo 架构，形成从 UI 到服务、从模型接入到工具执行的完整技术栈。通过统一的 TS/ESLint 配置、Turbo 构建与 pnpm 工作空间，项目在可维护性、可扩展性与开发效率方面具备良好基础。
