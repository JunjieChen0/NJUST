...
Roo-Code 是一个基于 VS Code 的 AI 编程助手扩展，专为 NJUST 内部使用而定制。该项目提供了完整的 AI 编程辅助功能，包括：
...
apps_web[apps/web-roo-code/<br/>Web 应用]
...
Roo-Code 采用了分层架构设计，实现了 UI、业务逻辑和服务层的有效分离：
...
3. **启动开发模式**：
   在 VS Code 中按 `F5` 启动调试，会打开一个加载了 NJUST_AI_CJ 扩展的新窗口。
...
MCP（Model Context Protocol）服务器是 Roo-Code 的重要功能组件：
...
subgraph "配置包"
ConfigTS[@njust-ai-cj/config-typescript<br/>TypeScript 配置]
end
Turbo --> VSCE
ESLint --> Prettier
Vitest --> TypeScript
...
Roo-Code 项目提供了完整的 AI 编程助手解决方案，具有以下优势：
