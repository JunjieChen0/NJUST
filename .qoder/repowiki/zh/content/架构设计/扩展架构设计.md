...
Roo-Code VS Code 扩展采用分离式架构设计，将扩展宿主（Extension Host）与 Webview 宿主进行清晰分离。该架构通过 ClineProvider 实现模式，构建了完整的任务执行引擎、服务注册机制和全局状态管理系统。
...
Roo-Code VS Code 扩展展现了现代扩展开发的最佳实践，通过分离式架构设计实现了高度模块化和可维护的代码结构。该架构的主要优势包括：
