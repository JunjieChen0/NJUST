...
## 简介
本指南面向首次接触 Roo-Code（NJUST_AI_CJ）项目的开发者与使用者，帮助你在最短时间内完成环境准备、安装配置与基础体验。你将学会：
...
## 项目结构
Roo-Code 采用 monorepo 结构，核心由 VS Code 扩展主体、React Webview 前端、CLI 工具与多个 Next.js 应用组成。关键目录与职责如下：
- src/：VS Code 扩展主体（核心逻辑、服务层、UI 与交互）
- webview-ui/：React 侧栏/WebView 前端
- apps/cli/：命令行工具 roo
- apps/web-roo-code/：官网/营销站点
...
subgraph "应用"
CLI["apps/cli/"]
WEB["apps/web-roo-code/"]
EVALS["apps/web-evals/"]
end
subgraph "共享包"
PKG["@njust-ai-cj/packages/*"]
end
Root --> SRC
Root --> WEBUI
Root --> CLI
...
- CLI 工具 roo：在命令行中启动交互式会话或非交互式输出，支持模式、模型、会话管理与认证。
...
## 架构概览
Roo-Code 的运行时分为三层：呈现层（Webview + VS Code UI）、扩展宿主（ClineProvider/Task）、服务层（MCP、索引、Cloud Agent、Skills、仓颉 LSP 等）。消息从 Webview 进入 Task，经 Provider API 推理，必要时执行工具，最终回到 UI。
...
- 基本用法
  ...
  - roo -p "你的提示"：非交互式打印输出
  - roo -c：继续当前工作区最近的任务
  - roo --session-id <id>：按会话 ID 恢复任务
- 列表命令
  - roo list commands/modes/models/sessions：列出可用条目
- 认证
  - roo auth login/logout/status：管理 Cloud 认证状态

```mermaid
sequenceDiagram
participant U as "用户"
participant CLI as "roo CLI"
participant EXT as "扩展核心"
participant API as "Provider API"
U->>CLI : roo "提示" 或 roo -p "提示"
CLI->>EXT : 初始化任务/会话
EXT->>API : 发送提示与历史
API-->>EXT : 返回文本/工具调用
EXT-->>CLI : 输出结果或等待审批
...
**图表来源**
- [apps/web-roo-code/package.json:5-15](file://apps/web-roo-code/package.json#L5-L15)
...
- apps/cli/package.json 定义了 roo 命令入口与依赖
- apps/web-roo-code/ 与 apps/web-evals/ 使用 Next.js，分别配置了不同的路由与重定向策略

```mermaid
graph LR
Root["根 package.json"] --> Engines["engines.node / packageManager"]
Root --> Scripts["脚本入口turbo"]
CLI["apps/cli/package.json"] --> Bin["bin: roo"]
WEB["apps/web-roo-code/package.json"] --> Next["Next.js 依赖"]
...
- 仓颉（Cangjie）工具链问题
  ...
  - 处理：设置 CANGJIE_HOME 或 njust-ai-cj.cangjieLsp.serverPath；验证工具路径与权限
...
## 结论
通过本快速开始指南，你已经完成了 Roo-Code 的环境准备、安装与基础使用。建议下一步：
- 在 VS Code 中体验扩展核心功能（对话、模式、工具审批）
- 使用 roo CLI 进行命令行交互与非交互式输出
- 部署 web-roo-code 或 web-evals 进行 Web 应用开发与评估
...
- CLI 基本用法：roo "提示" / roo -p "提示" / roo -c
- CLI 列表：roo list commands/modes/models/sessions
- CLI 认证：roo auth login/logout/status
- Web 开发：apps/web-roo-code/ 与 apps/web-evals/ 的 dev/build/start
