...
- [apps/web-roo-code/eslint.config.mjs](file://apps/web-roo-code/eslint.config.mjs)
...
- RooToolsMcpServer：扩展内嵌 MCP 服务，把部分 Roo 能力以 MCP 工具形式提供。
...
- 搜索服务：自然语言查询命中向量库，供 Agent 工具与 VS Code 注册的 roo_codebaseSearch 使用。
- 缓存：减少重复嵌入与重建成本。
...
### VS Code 内置 AI 集成
- Chat 参与者：注册 @roo 子命令（code / architect / ask / debug / plan 等），与内置聊天 UI 协作。
- Language Model 工具：roo_readFile、roo_editFile、roo_executeCommand、roo_searchFiles、roo_listFiles、roo_codebaseSearch。
...
### 代码规范与质量工具
- ESLint：src 与 webview-ui 分别使用 @njust-ai-cj/config-eslint 的基础与 React 配置；部分文件有特殊规则放宽。
- TypeScript：tsconfig.json 继承 @njust-ai-cj/config-typescript/base.json，包含必要的类型与排除。
...
- [apps/web-roo-code/eslint.config.mjs:1-5](file://apps/web-roo-code/eslint.config.mjs#L1-L5)
...
- 类型包发布：通过 npm:publish:types 脚本发布 @njust-ai-cj/types 包。
