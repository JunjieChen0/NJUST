...
- [apps/web-roo-code/src/actions/evals.ts](file://apps/web-roo-code/src/actions/evals.ts)
</cite>
...
- web-roo-code：评估结果聚合与展示（筛选、解析设置、格式化分数）
...
- [apps/web-roo-code/src/actions/evals.ts:1-30](file://apps/web-roo-code/src/actions/evals.ts#L1-L30)
...
- 关键模块职责：
  ...
  - web-roo-code/actions/evals.ts：聚合评估运行，校验设置、格式化分数与语言得分
...
participant Evals as "@njust-ai-cj/evals"
...
- 数据流与外部依赖
  - 依赖 @njust-ai-cj/evals 提供的运行/任务 CRUD 与查询接口
  - 使用 Redis 存储心跳与运行器集合
...
participant E as "@njust-ai-cj/evals"
...
### 组件六：评估运行聚合（web-roo-code/evals.ts）
- 功能要点
  - 获取所有运行，过滤已产生任务指标的运行
  - 使用 rooCodeSettingsSchema 校验设置有效性
...
## 依赖关系分析
- 内部依赖
  - runs.ts 依赖 @njust-ai-cj/evals 的运行/任务接口与并发工具 p-map
...
- [apps/web-evals/src/lib/server/redis.ts:1-14](file://apps/web-evals/src/lib/server/redis.ts#L1-L14)
- [apps/web-roo-code/src/actions/evals.ts:3-6](file://apps/web-roo-code/src/actions/evals.ts#L3-L6)
...
- 容器无法启动或无日志
  ...
  - 查看 /tmp/roo-code-evals.log 与容器日志
- Redis 连接失败
...
## 结论
本项目通过 Server Actions 将业务逻辑集中于服务端，结合 @njust-ai-cj/evals 的运行/任务模型、Docker 执行器、Redis 状态管理与文件系统存储，构建了完整的评估运行生命周期管理方案。建议在生产环境中强化输入校验、完善权限控制与审计日志，并持续优化并发与I/O策略。
