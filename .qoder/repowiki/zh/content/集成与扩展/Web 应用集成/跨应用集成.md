...
本项目是一个跨应用集成解决方案，主要包含两个Web应用：主Web应用（web-roo-code）和评估Web应用（web-evals）。该项目的核心目标是实现两个应用之间的数据共享、统一认证系统和状态同步方案。
...
graph TB
subgraph "应用层"
WebRooCode[主Web应用<br/>web-roo-code]
WebEvals[评估Web应用<br/>web-evals]
end
subgraph "共享包层"
Types[类型定义包<br/>@njust-ai-cj/types]
Core[核心功能包<br/>@njust-ai-cj/core]
Evals[评估包<br/>@njust-ai-cj/evals]
IPC[IPC通信包<br/>@njust-ai-cj/ipc]
end
subgraph "基础设施"
NextJS[Next.js框架]
...
graph TB
subgraph "主Web应用依赖"
WebRooCodeDep[web-roo-code]
EvalDep[@njust-ai-cj/evals]
TypeDep[@njust-ai-cj/types]
...
EvalDep2[@njust-ai-cj/evals]
TypeDep2[@njust-ai-cj/types]
HookFormDep[react-hook-form]
RedisDep[redis]
CmdkDep[cmdk]
end
subgraph "共享依赖"
CoreDep[@njust-ai-cj/core]
IPCDep[@njust-ai-cj/ipc]
ZodDep[zod]
PostCSSDep[postcss]
end
WebRooCodeDep --> EvalDep
WebRooCodeDep --> TypeDep
WebRooCodeDep --> CoreDep
WebRooCodeDep --> IPCDep
