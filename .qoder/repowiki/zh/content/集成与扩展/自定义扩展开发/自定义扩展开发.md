...
本指南面向需要开发自定义扩展的开发者，详细介绍Roo-Code项目中Model Context Protocol (MCP)协议的实现、MCP服务器架构和工具执行器的设计模式。文档涵盖了从基础概念到高级应用的完整开发流程，包括：
...
本指南详细介绍了Roo-Code项目中MCP协议的完整实现，包括客户端管理、服务器架构、工具执行器设计和安全机制。通过理解这些核心组件的工作原理，开发者可以：
