...
## How NJUST_AI_CJ captures the why automatically

When you build with NJUST_AI_CJ, you get decision documentation as a side effect of the workflow. Every task generates a log of what was tried, what failed, and what worked. The diff history shows the evolution. PR comments capture the reasoning. The prototype becomes the specification, and the trail becomes the decision record.

This addresses the "lost rationale" failure mode without requiring a separate documentation step. The institutional memory accumulates automatically because NJUST_AI_CJ iterates: it proposes changes, runs tests, observes results, and keeps iterating until the code passes. All of that is logged.
...
### How does NJUST_AI_CJ help with prototype-first workflows?

NJUST_AI_CJ connects idea to working prototype. You describe what you want; NJUST_AI_CJ proposes changes, runs tests, observes results, and iterates. The task log captures decisions automatically. The result is a working prototype with a built-in decision trail-no separate documentation step required.
